...
// Somente a lista de repositórios. Nada de gráficos.
...
export function renderRepos(repos) {
  ...
  if (list.length === 0) {
    setHtml('#repos', '<p class="empty">Nenhum repositório público encontrado.</p>');
    return;
  }
  ...
  setHtml(
    ...
    list
      ...
      <a class="repo" href="${repo.html_url}" target="_blank" rel="noopener noreferrer">
        ...
        <p class="repo-desc">${escapeHtml(repo.description || 'Sem descrição')}</p>
        <div class="repo-meta">
